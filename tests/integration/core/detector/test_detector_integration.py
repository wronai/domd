...
class TestDetectorIntegration:
    ...
    def test_mixed_project_types(self, temp_project, sample_package_json):
        ...
        with patch(
            ...
            with patch("domd.core.project_detection.detector.CommandHandler") as mock_handler:
                mock_instance = mock_handler.return_value
                mock_instance.parse_commands.side_effect = lambda cmds: cmds  # Just return the commands as-is
                
                # Scan the project
                commands = detector.scan_project()
                
                # Verify we found commands from both files
                assert len(commands) == 3, f"Expected 3 commands, got {len(commands)}: {commands}"
                
                # Convert commands to dict for easier assertion
                cmd_dicts = [cmd if isinstance(cmd, dict) else cmd.__dict__ for cmd in commands]
                
                assert any(cmd.get("command") == "npm test" for cmd in cmd_dicts), "npm test command not found"
                assert any(cmd.get("command") == "make test" for cmd in cmd_dicts), "make test command not found"
                assert any(cmd.get("command") == "make build" for cmd in cmd_dicts), "make build command not found"
    ...
    def test_exclude_patterns_integration(self, temp_project):
        ...
        with patch(
            ...
            with patch("domd.core.project_detection.detector.CommandHandler") as mock_handler:
                ...
                assert "include.txt" in cmd_dicts[0]["source"], "Excluded file was processed"
                
                # Verify the config handler was called with the correct exclude patterns
                assert hasattr(detector.config_handler, 'exclude_patterns'), "Config handler missing exclude_patterns"
                assert "exclude.*" in detector.config_handler.exclude_patterns, "Exclude pattern not set correctly"
