...
class TestCommandHandler:
    ...
    def test_error_handling_in_pattern_matching(self, handler, caplog):
        ...
        with patch("re.search", side_effect=Exception("Test error")):
            is_valid, reason = handler.is_valid_command("test")
            assert "Error checking pattern" in caplog.text
            assert (
                is_valid is False
            ), "Expected invalid command due to pattern matching error"
